...
public class listenPortByNum60x {
	public static void LPBN6(Socket s, String jsonString, Integer key) {
		Socket socket = s;
	//	System.out.println(clocker.getTime()+"服务端消息:正在转发消息 " + jsonString);
		...
		if ((mSocket = playAndChessBoard.getOpSocket(socket, key)) != null) {
			...
			try {
				...
				// 刷新缓存区
				pout.flush();
		//				clocker.getTime()+"服务端消息:已成功转发消息" + jsonString + "至:" + mSocket.getInetAddress() + ":" + mSocket.getPort());
			} catch (IOException e) {
				//e.printStackTrace();
				System.out.println(clocker.getTime()+"服务端消息:消息转发失败，服务器已断开连接");
			} // try
		} else {
